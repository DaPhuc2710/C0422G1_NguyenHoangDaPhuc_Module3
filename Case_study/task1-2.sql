...
USE case_study;
CREATE TABLE vi_tri (
    ma_vi_tri INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT,
    ten_vi_tri VARCHAR(45)
);

CREATE TABLE trinh_do (
    ma_trinh_do INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT,
    ten_trinh_do VARCHAR(45)
);

CREATE TABLE bo_phan (
    ma_bo_phan INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT,
    ten_bo_phan VARCHAR(45)
);

CREATE TABLE loai_khach (
    ma_loai_khach INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT,
    ten_loai_khach VARCHAR(45)
);

CREATE TABLE loai_dich_vu (
    ma_loai_dich_vu INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT,
    ten_loai_dich_vu VARCHAR(45)
);

CREATE TABLE kieu_thue (
    ma_kieu_thue INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT,
    ten_kieu_thue VARCHAR(45)
);

CREATE TABLE dich_vu_di_kem (
    ma_dich_vu_di_kem INT NOT NULL UNIQUE PRIMARY KEY AUTO_INCREMENT,
...
("Thuê xe máy","10000","chiếc","hỏng 1 chiếc"),
...
insert into dich_vu(ten_dich_vu,dien_tich,chi_phi_thue,so_nguoi_toi_da,tieu_chuan_phong,mo_ta_tien_nghi_khach,dien_tich_ho_boi,so_tang,dich_vu_mien_phi_di_kem,ma_kieu_thue,ma_loai_dich_vu)
values
...
where nv.ho_ten regexp "^(\\S+(\\s\\S+)*(\\s(H|T|K)\\S+)+)$" 
...
where kh.dia_chi like "% Đà Nẵng" or kh.dia_chi like "% Quảng Trị" and (year(current_date())-year(kh.ngay_sinh)) between 18 and 50;
